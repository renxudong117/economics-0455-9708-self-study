...
## 第 1 课｜5.1 数据类型、均值与方差｜Data types, mean and variance

### 深度展开与真题化应用（共 10 个知识点）

#### 知识点 01/10｜概念从哪里开始｜Starting from the concept

统计从 **population（总体）** 与 **sample（样本）** 开始。数据可以是 discrete（离散，如人数）或 continuous（连续，如时间）；可以是 raw data、frequency data 或 grouped data。先判断数据类型，才能选择图表和公式。

#### 知识点 02/10｜公式、符号与每一项含义｜Formula and notation

未分组数据：mean = Σx/n。频数表：mean = Σfx/Σf。总体方差常写 Var(X)=Σ(x-mean)²/n，也可用更快的 Σx²/n-(Σx/n)²；频数表对应 Σfx²/Σf-(Σfx/Σf)²。standard deviation 是方差的正平方根。

#### 知识点 03/10｜公式为什么成立｜Why the formula works

方差把每个值与均值的离差平方后取平均。平方有两个作用：正负离差不会抵消，而且离均值更远的值被更重地惩罚。快捷公式由展开 (x-mean)² 得到，所以两式计算同一个量。

#### 知识点 04/10｜学生可直接执行的方法｜Step-by-step student method

写表格列 x、f、fx、fx²。先算 Σf 确认总频数；再算 Σfx 得均值；接着算 Σfx²；最后代入方差公式。不要把 standard deviation 与 variance 混写，前者与原数据同单位，后者是平方单位。

#### 知识点 05/10｜例题一：基础代入全过程｜Worked example 1
...
#### 知识点 06/10｜例题二：进阶应用全过程｜Worked example 2

**题目｜Question.** x为1,2,3,4，频数分别2,5,4,1，求mean。

**正确答案｜Correct answer.** Σf=12；Σfx=2+10+12+4=28；mean=28/12=7/3≈2.33。规范答案先给精确分数，再按要求给小数。

#### 知识点 07/10｜图像或模型怎样读｜Reading the representation

下面的图用于建立本课的视觉模型。先读横轴/事件或位置，再读高度、面积、分支或中心；最后把图形信息翻译成公式。
...
#### 知识点 08/10｜计算器应该在哪一步使用｜Using the calculator correctly

先在纸面写出随机变量、分布、参数、公式或概率事件，再用计算器完成数值运算。计算器负责 arithmetic，不负责决定用 nPr 还是 nCr、不负责判断独立性，也不会替你做连续性修正。

#### 知识点 09/10｜高频错误与修正｜Common error and repair

计算器的 σx 与 sx 可能分别表示总体和样本标准差。Cambridge 题目要求哪一种取决于题设与考纲口径；手算时必须依照给定公式。分组数据只能用组中值估计，因此答案要写 estimated mean。

#### 知识点 10/10｜大考书写与最终答案｜Exam presentation

完整答案按“定义对象 → 写公式/分布与条件 → 代入 → 化简/求概率 → 精度和单位 → 语境结论”排列。show that 题必须达到指定形式；exact 题保留分数、根式或常数；概率通常给合适有效数字。

### 逐题精讲与评价拓展（共 9 项）

#### 精讲 01/09｜基础概念应用 1｜Applied concept
...
#### 精讲 02/09｜公式推导与代入 2｜Formula derivation

**题目｜Question.** For the data 2, 4, 4, 6, derive the variance using both forms of the formula.
...
#### 精讲 03/09｜完整例题 3｜Fully worked example 1

**题目｜Question.** 数据 3,5,5,7,10 的 mean 和 variance。

**完整解答｜Full solution.** **第 1 步｜Step 1.** 识别题目对象与已知条件。 **第 2 步｜Step 2.** 选择本课对应公式或计数结构。 **第 3 步｜Step 3.** Σx=30，n=5，所以 mean=30/5=6。Σx²=9+25+25+49+100=208。variance=208/5-6²=41.6-36=5.6；sd=√5.6≈2.37。

#### 精讲 04/09｜完整例题 4｜Fully worked example 2

**题目｜Question.** x为1,2,3,4，频数分别2,5,4,1，求mean。

**完整解答｜Full solution.** **第 1 步｜Step 1.** 把文字条件翻译成符号、位置或事件。 **第 2 步｜Step 2.** 逐项代入并保留中间值。 **第 3 步｜Step 3.** Σf=12；Σfx=2+10+12+4=28；mean=28/12=7/3≈2.33。规范答案先给精确分数，再按要求给小数。

#### 精讲 05/09｜选择题 5｜Multiple-choice question

**题目｜Question.** For values 1,2,4 with frequencies 3,2,1, what is the mean?
A 1.5  B 2.0  C 11/6  D 7/3

**正确答案｜Correct answer.** C - 11/6. Σfx=3+4+4=11 and Σf=6, so mean=11/6.

#### 精讲 06/09｜结构题 6｜Structured question
...
#### 精讲 07/09｜迁移题 7｜Transfer question
...
#### 精讲 08/09｜具体错解订正 8｜Worked error correction
...
#### 精讲 09/09｜英文答题 9｜English exam response

**题目｜Question.** A set has mean 12 and variance 9. Every value is changed by y=2x+5. Explain in English how the two statistics change.

**参考答案｜Model answer.** Adding 5 shifts every observation equally, so it raises the mean but does not change the spread. Multiplying by 2 doubles every deviation from the mean, so the variance is multiplied by 2². Hence E(Y)=2(12)+5=29 and Var(Y)=4(9)=36.

## 第 2 课｜5.1 茎叶图、四分位数与箱线图｜Stem-and-leaf and box plots

### 深度展开与真题化应用（共 10 个知识点）

#### 知识点 01/10｜概念从哪里开始｜Starting from the concept

ordered stem-and-leaf diagram 保留每个原始值并按大小排列；box-and-whisker plot 用 minimum、Q1、median、Q3、maximum 概括分布。IQR=Q3-Q1 描述中间50%数据的离散程度。

#### 知识点 02/10｜公式、符号与每一项含义｜Formula and notation

有 n 个数据时，中位数位置可用 (n+1)/2 辅助定位，但四分位数位置要按题目所采用的排序规则读取。考试常从给定图直接读五数概括，不要求争论不同软件定义。

#### 知识点 03/10｜公式为什么成立｜Why the formula works

中位数把有序数据分成两半；Q1 是下半部分的中位数，Q3 是上半部分的中位数。IQR只使用中间一半数据，因此极端值变化往往不影响IQR，却可能明显改变range。

#### 知识点 04/10｜学生可直接执行的方法｜Step-by-step student method

先把数据从小到大排列。画茎叶图时写 key，例如 3|7 means 37。画箱线图时先设统一刻度，标五数概括；箱体从Q1到Q3，中间线是median，两侧whiskers延伸到minimum和maximum。
...
**题目｜Question.** A组median=52,IQR=8；B组median=47,IQR=15。如何比较？

**正确答案｜Correct answer.** A has a higher typical value because its median is 52 rather than 47. A is more consistent because its IQR is smaller, 8 rather than 15. 两个结论都必须引用数值。

#### 知识点 07/10｜图像或模型怎样读｜Reading the representation

下面的图用于建立本课的视觉模型。先读横轴/事件或位置，再读高度、面积、分支或中心；最后把图形信息翻译成公式。

**English explanation.** A box plot separates location from spread. The median compares typical values; the IQR compares the spread of the middle half. A longer whisker or an off-centre median can suggest skewness, but a box plot does not reveal every individual value.
...
**题目｜Question.** Explain why the IQR is often preferred to the range when a distribution contains an extreme value.
...
**完整解答｜Full solution.** **第 1 步｜Step 1.** 识别题目对象与已知条件。 **第 2 步｜Step 2.** 选择本课对应公式或计数结构。 **第 3 步｜Step 3.** n=9，median为第5个值10。下半4,7,8,8的Q1=(7+8)/2=7.5；上半13,15,19,22的Q3=(15+19)/2=17；IQR=9.5。

#### 精讲 04/09｜完整例题 4｜Fully worked example 2

**题目｜Question.** A组median=52,IQR=8；B组median=47,IQR=15。如何比较？

**完整解答｜Full solution.** **第 1 步｜Step 1.** 把文字条件翻译成符号、位置或事件。 **第 2 步｜Step 2.** 逐项代入并保留中间值。 **第 3 步｜Step 3.** A has a higher typical value because its median is 52 rather than 47. A is more consistent because its IQR is smaller, 8 rather than 15. 两个结论都必须引用数值。

#### 精讲 05/09｜选择题 5｜Multiple-choice question

**题目｜Question.** Which statistic is least affected by one extremely large value?
A mean  B range  C median  D standard deviation

**正确答案｜Correct answer.** C - median. Its position in the ordered list may be unchanged, while the mean, range and standard deviation respond to the extreme value.
...
**题目｜Question.** If 5 is added to every observation, what happens to median and IQR?
...
**题目｜Question.** Two box plots show delivery times. A has median 18 minutes and IQR 5 minutes; B has median 15 minutes and IQR 12 minutes. Compare them in English.

**参考答案｜Model answer.** Company B is typically faster because its median delivery time is lower, 15 minutes rather than 18 minutes. However, company A is more consistent because its interquartile range is smaller, 5 minutes rather than 12 minutes.
...
**题目｜Question.** Derive the formula frequency = class width × frequency density from the meaning of histogram area.
...
**完整解答｜Full solution.** **第 1 步｜Step 1.** 识别题目对象与已知条件。 **第 2 步｜Step 2.** 选择本课对应公式或计数结构。 **第 3 步｜Step 3.** 第一组宽5，density=20/5=4。第二组宽10，density=30/10=3。虽然第二组频数更大，柱高反而较低，因为柱更宽。

#### 精讲 04/09｜完整例题 4｜Fully worked example 2

**题目｜Question.** 20≤x<30的柱高2.4；估计22≤x<27的频数。

**完整解答｜Full solution.** **第 1 步｜Step 1.** 把文字条件翻译成符号、位置或事件。 **第 2 步｜Step 2.** 逐项代入并保留中间值。 **第 3 步｜Step 3.** 整组密度2.4，子区间宽5，所以估计频数=2.4×5=12。这里使用组内均匀分布假设。

#### 精讲 05/09｜选择题 5｜Multiple-choice question

**题目｜Question.** A class has width 8 and frequency 36. What is its frequency density?
A 4  B 4.5  C 28  D 288

**正确答案｜Correct answer.** B - 4.5, because 36/8=4.5.
...
**题目｜Question.** Two bars have equal height but widths 5 and 12. Compare frequencies.
...
**题目｜Question.** One histogram bar covers 40≤t<60 at density 2.5. Estimate the number with 48≤t<56 and explain the assumption.

**参考答案｜Model answer.** The required subinterval has width 56−48=8, so the estimated frequency is 8×2.5=20. This assumes observations are distributed uniformly within the class interval.
...
**题目｜Question.** Show why nPr=n!/(n−r)! rather than n!/r!.
...
**完整解答｜Full solution.** **第 1 步｜Step 1.** 识别题目对象与已知条件。 **第 2 步｜Step 2.** 选择本课对应公式或计数结构。 **第 3 步｜Step 3.** 职位不同，顺序重要。第一职8选1，第二职剩7人，第三职剩6人：8×7×6=336。也可写 8P3=8!/(8-3)!=8!/5!=336。

#### 精讲 04/09｜完整例题 4｜Fully worked example 2

**题目｜Question.** 排列单词 LEVEL 的字母，有多少种不同排列？

**完整解答｜Full solution.** **第 1 步｜Step 1.** 把文字条件翻译成符号、位置或事件。 **第 2 步｜Step 2.** 逐项代入并保留中间值。 **第 3 步｜Step 3.** 共有5个位置，若字母都不同有5!。L重复2次、E重复2次，交换相同字母不产生新排列，所以数量=5!/(2!2!)=120/4=30。

#### 精讲 05/09｜选择题 5｜Multiple-choice question

**题目｜Question.** Eight students fill three different offices. How many allocations are possible?
A 8C3  B 3^8  C 8P3  D 8!/3!

**正确答案｜Correct answer.** C - 8P3=8×7×6=336, because the three offices are distinct.
...
**题目｜Question.** A student calculates 8C3 for president, secretary and treasurer. Explain the error and give the correct answer.
...
**题目｜Question.** Explain in English why LEVEL has 5!/(2!2!) different arrangements.

**参考答案｜Model answer.** If all five letters were distinct, there would be 5! arrangements. The two Ls can be exchanged in 2! ways without creating a new word, and the two Es can also be exchanged in 2! invisible ways. Therefore the number of distinct arrangements is 5!/(2!2!)=30.
...
**题目｜Question.** Eight people are available and three are chosen as an unranked committee. Derive the answer without treating nCr as a calculator key.
...
**题目｜Question.** From 6 women and 5 men, choose a committee of 4 containing exactly 2 women. Give every factor a meaning.
...
**完整解答｜Full solution.** **第 1 步｜Step 1.** 识别题目对象与已知条件。 **第 2 步｜Step 2.** 选择本课对应公式或计数结构。 **第 3 步｜Step 3.** 顺序不重要，用组合。8C3=8!/[3!(8-3)!]=8!/(3!5!)=(8×7×6)/(3×2×1)=56。规范写法：There are **56 possible committees**。

#### 精讲 04/09｜完整例题 4｜Fully worked example 2

**题目｜Question.** 5男4女中选3人，至少1女，有多少种？

**完整解答｜Full solution.** **第 1 步｜Step 1.** 把文字条件翻译成符号、位置或事件。 **第 2 步｜Step 2.** 逐项代入并保留中间值。 **第 3 步｜Step 3.** 总选法9C3=84。减去全男5C3=10，所以至少1女=84-10=74。也可分1女、2女、3女相加，但补集更短。

#### 精讲 05/09｜选择题 5｜Multiple-choice question

**题目｜Question.** How many committees of 3 can be selected from 8 people?
A 24  B 56  C 336  D 512

**正确答案｜Correct answer.** B - 56. Order does not matter, so 8C3=8!/(3!5!)=56.
...
**题目｜Question.** A student answers “at least one girl” with 4C1×5C2. Identify what was counted and complete the correct calculation for 4 girls and 5 boys choosing 3.
...
**题目｜Question.** A committee of 3 is chosen from 8 people. Explain in English why the answer is 56 and not 336.

**参考答案｜Model answer.** A committee has no different positions, so changing the order in which the same three people are selected does not create a new committee. The ordered count 8P3=336 counts each trio 3!=6 times. Dividing by 6 gives 8C3=56 distinct committees.
...
**题目｜Question.** A and B are independent with P(A)=0.4 and P(B)=0.7. Find the probability that exactly one occurs.
...
**完整解答｜Full solution.** **第 1 步｜Step 1.** 识别题目对象与已知条件。 **第 2 步｜Step 2.** 选择本课对应公式或计数结构。 **第 3 步｜Step 3.** 0.55+0.40-0.18=0.77。必须减交集，否则得到0.95并重复计算同时发生部分。

#### 精讲 04/09｜完整例题 4｜Fully worked example 2

**题目｜Question.** A、B独立，P(A)=0.3，P(B)=0.6，求恰好一个发生。

**完整解答｜Full solution.** **第 1 步｜Step 1.** 把文字条件翻译成符号、位置或事件。 **第 2 步｜Step 2.** 逐项代入并保留中间值。 **第 3 步｜Step 3.** P(A∩B′)+P(A′∩B)=0.3×0.4+0.7×0.6=0.12+0.42=0.54。两条路径互斥，所以相加。

#### 精讲 05/09｜选择题 5｜Multiple-choice question

**题目｜Question.** A and B are independent with probabilities 0.4 and 0.5. Find P(A∩B).
A 0  B 0.2  C 0.9  D 1

**正确答案｜Correct answer.** B - 0.2, because independence gives 0.4×0.5.
...
**题目｜Question.** Find P(neither A nor B) when P(A∪B)=0.77.
...
**题目｜Question.** Explain in English the difference between “mutually exclusive” and “independent”.

**参考答案｜Model answer.** Mutually exclusive events cannot occur together, so their intersection has probability zero. Independent events may occur together, but knowing that one occurred does not change the probability of the other; equivalently P(A∩B)=P(A)P(B).
...
**题目｜Question.** Use the definition of conditional probability when P(A∩B)=0.18 and P(B)=0.30.
...
**完整解答｜Full solution.** **第 1 步｜Step 1.** 识别题目对象与已知条件。 **第 2 步｜Step 2.** 选择本课对应公式或计数结构。 **第 3 步｜Step 3.** 红红=4/7×3/6=2/7；蓝蓝=3/7×2/6=1/7；同色=3/7。两条路径互斥，所以相加。

#### 精讲 04/09｜完整例题 4｜Fully worked example 2

**题目｜Question.** 某病患病率0.02；检测对患病者阳性0.95，对未患病者假阳性0.04。求阳性概率。

**完整解答｜Full solution.** **第 1 步｜Step 1.** 把文字条件翻译成符号、位置或事件。 **第 2 步｜Step 2.** 逐项代入并保留中间值。 **第 3 步｜Step 3.** P(+)=0.02×0.95+0.98×0.04=0.019+0.0392=0.0582。若问P(disease|+)，再算0.019/0.0582≈0.326。

#### 精讲 05/09｜选择题 5｜Multiple-choice question

**题目｜Question.** A bag has 3 white and 2 black balls. Two are drawn without replacement. Find P(white then black).
A 1/5  B 3/10  C 2/5  D 1/2

**正确答案｜Correct answer.** B - 3/5×2/4=3/10.

#### 精讲 06/09｜结构题 6｜Structured question

**题目｜Question.** Given P(A∩B)=0.24 and P(B)=0.4, find P(A|B).

**完整解答｜Full solution.** P(A|B)=0.24/0.4=0.6. The denominator is the probability of the given event B.

#### 精讲 07/09｜迁移题 7｜Transfer question

**题目｜Question.** When can a without-replacement model be approximated as independent?
...
**题目｜Question.** Derive Var(aX+b)=a²Var(X) from deviations from the mean.

**完整解答｜Full solution.** E(aX+b)=aE(X)+b. Therefore Y−E(Y)=aX+b−[aE(X)+b]=a[X−E(X)]. Squaring gives (Y−E(Y))²=a²[X−E(X)]²; taking expectations gives Var(Y)=a²Var(X).

#### 精讲 03/09｜完整例题 3｜Fully worked example 1

**题目｜Question.** X取0,1,2，概率k,2k,3k。求k与E(X)。

**完整解答｜Full solution.** **第 1 步｜Step 1.** 识别题目对象与已知条件。 **第 2 步｜Step 2.** 选择本课对应公式或计数结构。 **第 3 步｜Step 3.** 6k=1，所以k=1/6。E(X)=0+1×2/6+2×3/6=4/3。

#### 精讲 04/09｜完整例题 4｜Fully worked example 2

**题目｜Question.** 上一分布求variance。

**完整解答｜Full solution.** **第 1 步｜Step 1.** 把文字条件翻译成符号、位置或事件。 **第 2 步｜Step 2.** 逐项代入并保留中间值。 **第 3 步｜Step 3.** E(X²)=0+1×2/6+4×3/6=7/3。Var=7/3-(4/3)²=5/9。

#### 精讲 05/09｜选择题 5｜Multiple-choice question

**题目｜Question.** E(X)=5 and Var(X)=4. For Y=3X-2, find E(Y).
A 7  B 13  C 15  D 19

**正确答案｜Correct answer.** B - 13, because E(3X-2)=3×5-2.

#### 精讲 06/09｜结构题 6｜Structured question

**题目｜Question.** For the same transformation, find Var(Y).

**完整解答｜Full solution.** Var(3X-2)=3²Var(X)=9×4=36. The subtraction of 2 does not affect variance.

#### 精讲 07/09｜迁移题 7｜Transfer question

**题目｜Question.** A game costs $3 and pays $10 with probability 0.2, otherwise $0. Find expected net gain.
...
**题目｜Question.** Explain why E(X)=np is plausible for a binomial count.
...
**完整解答｜Full solution.** **第 1 步｜Step 1.** 识别题目对象与已知条件。 **第 2 步｜Step 2.** 选择本课对应公式或计数结构。 **第 3 步｜Step 3.** 令X为正面次数，X~B(8,0.4)。P(X=3)=8C3(0.4)^3(0.6)^5≈0.2787，答案0.279。

#### 精讲 04/09｜完整例题 4｜Fully worked example 2

**题目｜Question.** 同一模型，至少1次正面。

**完整解答｜Full solution.** **第 1 步｜Step 1.** 把文字条件翻译成符号、位置或事件。 **第 2 步｜Step 2.** 逐项代入并保留中间值。 **第 3 步｜Step 3.** P(X≥1)=1-P(X=0)=1-(0.6)^8≈0.9832。用补集只需一项。

#### 精讲 05/09｜选择题 5｜Multiple-choice question

**题目｜Question.** X~B(10,0.2). Which expression gives P(X≥1)?
A 0.2^10  B 1-0.2^10  C 1-0.8^10  D 10×0.2

**正确答案｜Correct answer.** C - 1-P(X=0)=1-(0.8)^10.

#### 精讲 06/09｜结构题 6｜Structured question

**题目｜Question.** A machine makes a defective item with probability 0.03 independently. Find probability exactly 2 of 20 are defective.

**完整解答｜Full solution.** X~B(20,0.03). P(X=2)=20C2(0.03)^2(0.97)^18≈0.0988.

#### 精讲 07/09｜迁移题 7｜Transfer question

**题目｜Question.** Find the mean and standard deviation for B(80,0.35).
...
**题目｜Question.** 同一模型，5次内至少成功一次。

**正确答案｜Correct answer.** P(X≤5)=1-P(前5次全失败)=1-0.75^5≈0.7627。
...
**题目｜Question.** Derive P(X≤r)=1−(1−p)^r.
...
**完整解答｜Full solution.** **第 1 步｜Step 1.** 识别题目对象与已知条件。 **第 2 步｜Step 2.** 选择本课对应公式或计数结构。 **第 3 步｜Step 3.** 前三次失败、第四次成功：P(X=4)=0.75^3×0.25≈0.1055。

#### 精讲 04/09｜完整例题 4｜Fully worked example 2

**题目｜Question.** 同一模型，5次内至少成功一次。

**完整解答｜Full solution.** **第 1 步｜Step 1.** 把文字条件翻译成符号、位置或事件。 **第 2 步｜Step 2.** 逐项代入并保留中间值。 **第 3 步｜Step 3.** P(X≤5)=1-P(前5次全失败)=1-0.75^5≈0.7627。

#### 精讲 05/09｜选择题 5｜Multiple-choice question

**题目｜Question.** With p=0.3, find the probability that the first success is after trial 4.
A 0.3^4  B 0.7^3×0.3  C 0.7^4  D 1-0.7^4

**正确答案｜Correct answer.** C - 0.7^4, because the first four trials must all fail.

#### 精讲 06/09｜结构题 6｜Structured question

**题目｜Question.** Find P(X=6) when p=0.2.

**完整解答｜Full solution.** P(X=6)=0.8^5×0.2=0.065536≈0.0655.

#### 精讲 07/09｜迁移题 7｜Transfer question

**题目｜Question.** Explain the memoryless property.
...
**题目｜Question.** Explain why standardisation uses (X−μ)/σ rather than (X−μ)/σ².
...
**完整解答｜Full solution.** **第 1 步｜Step 1.** 识别题目对象与已知条件。 **第 2 步｜Step 2.** 选择本课对应公式或计数结构。 **第 3 步｜Step 3.** z=(56-50)/3=2。P(X<56)=P(Z<2)=0.9772。56在均值右侧2个sd，左侧概率应大于0.5，检查通过。

#### 精讲 04/09｜完整例题 4｜Fully worked example 2

**题目｜Question.** 同一分布，求a使P(X<a)=0.10。

**完整解答｜Full solution.** **第 1 步｜Step 1.** 把文字条件翻译成符号、位置或事件。 **第 2 步｜Step 2.** 逐项代入并保留中间值。 **第 3 步｜Step 3.** 下尾10%对应z≈-1.2816。a=50+(-1.2816)×3≈46.2。下尾界值应小于均值，检查通过。

#### 精讲 05/09｜选择题 5｜Multiple-choice question

**题目｜Question.** X~N(100,16). What is the standard deviation?
A 4  B 8  C 16  D 256

**正确答案｜Correct answer.** A - 4, because the second parameter is variance and √16=4.

#### 精讲 06/09｜结构题 6｜Structured question

**题目｜Question.** X~N(80,25). Find P(72<X<88).

**完整解答｜Full solution.** σ=5. z values are -1.6 and 1.6. Probability=P(-1.6<Z<1.6)=2Φ(1.6)-1≈0.890.

#### 精讲 07/09｜迁移题 7｜Transfer question

**题目｜Question.** Find the 95th percentile of N(40,6²).
...
**题目｜Question.** Explain continuity correction for P(30≤X≤45) using discrete bars.
...
**完整解答｜Full solution.** **第 1 步｜Step 1.** 识别题目对象与已知条件。 **第 2 步｜Step 2.** 选择本课对应公式或计数结构。 **第 3 步｜Step 3.** np=80，nq=120，条件合适。Y~N(80,48)。P(X≤70)≈P(Y<70.5)。z=(70.5-80)/√48≈-1.371，概率≈0.0851。

#### 精讲 04/09｜完整例题 4｜Fully worked example 2

**题目｜Question.** 同一模型，近似求P(75≤X≤90)。

**完整解答｜Full solution.** **第 1 步｜Step 1.** 把文字条件翻译成符号、位置或事件。 **第 2 步｜Step 2.** 逐项代入并保留中间值。 **第 3 步｜Step 3.** 修正为P(74.5<Y<90.5)。z1≈-0.794，z2≈1.516。概率=Φ(1.516)-Φ(-0.794)≈0.722。

#### 精讲 05/09｜选择题 5｜Multiple-choice question

**题目｜Question.** Which is the corrected event for P(X<12)?
A Y<11.5  B Y<12  C Y<12.5  D Y>11.5

**正确答案｜Correct answer.** A - X<12 means X≤11, so use Y<11.5.

#### 精讲 06/09｜结构题 6｜Structured question

**题目｜Question.** Use a normal approximation for X~B(100,0.5) to find P(X≥60).

**完整解答｜Full solution.** Y~N(50,25). Correct to P(Y>59.5). z=(59.5-50)/5=1.9, so probability≈1-Φ(1.9)=0.0287.

#### 精讲 07/09｜迁移题 7｜Transfer question

**题目｜Question.** Why must the final answer be labelled approximate?
